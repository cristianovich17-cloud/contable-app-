  ...
  return (
    <div className="min-h-screen bg-gray-900 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
          <div className="bg-gray-800 border border-gray-700 rounded-lg shadow p-12 text-center text-gray-300">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-gray-800 border border-gray-700 rounded-lg shadow p-6">
                ...
                <p className={`text-2xl font-bold ${reporteMensual.balance >= 0 ? 'text-blue-400' : 'text-red-400'}`}>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              <div className="bg-gray-800 border border-gray-700 rounded-lg shadow p-6">
                <h3 className="text-lg font-bold mb-4 text-green-400">Ingresos por Categoría</h3>
                <div className="space-y-2">
                  {Object.entries(reporteMensual.ingresos.porCategoria).map(([cat, monto]) => (
                    <div key={cat} className="flex justify-between pb-2 border-b border-gray-700">
                      <span className="capitalize text-gray-300">{cat}</span>
                      <span className="font-semibold text-gray-200">${(monto as number).toFixed(2)}</span>
                    </div>
                  ))}
                  <div className="flex justify-between pt-2 font-bold text-green-400">
                    <span>Total</span>
              ...
              <div className="bg-gray-800 border border-gray-700 rounded-lg shadow p-6">
                <h3 className="text-lg font-bold mb-4 text-red-400">Egresos por Categoría</h3>
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between pt-2 font-bold text-red-400">
                    <span>Total</span>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-gray-800 border border-gray-700 rounded-lg shadow p-6">
                <p className="text-gray-400 text-sm">Total Ingresos Anuales</p>
                <p className="text-2xl font-bold text-green-400">
                  ${reporteAnual.totales.totalIngresosAnual.toFixed(2)}
                </p>
              </div>
              <div className="bg-gray-800 border border-gray-700 rounded-lg shadow p-6">
                <p className="text-gray-400 text-sm">Total Egresos Anuales</p>
                <p className="text-2xl font-bold text-red-400">
                  ${reporteAnual.totales.totalEgresosAnual.toFixed(2)}
                </p>
              </div>
              <div className="bg-gray-800 border border-gray-700 rounded-lg shadow p-6">
                <p className="text-gray-400 text-sm">Balance Anual</p>
                <p className={`text-2xl font-bold ${reporteAnual.totales.balanceAnual >= 0 ? 'text-blue-400' : 'text-red-400'}`}>
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-lg shadow p-6">
              <h3 className="text-lg font-bold mb-4 text-white">Resumen Mensual {año}</h3>
                  ...
                  <tbody className="divide-y divide-gray-700">
                    {reporteAnual.resumenMensual?.map((m: any, idx: number) => (
                      <tr key={idx} className="hover:bg-gray-700">
                        ...
                        <td className="px-4 py-2 text-right text-green-400">${m.totalIngresos.toFixed(2)}</td>
                        <td className="px-4 py-2 text-right text-red-400">${m.totalEgresos.toFixed(2)}</td>
                        <td className={`px-4 py-2 text-right ${m.balance >= 0 ? 'text-blue-400' : 'text-red-400'}`}>
                          ${m.balance.toFixed(2)}
                        </td>
                      </tr>
                    ))}
